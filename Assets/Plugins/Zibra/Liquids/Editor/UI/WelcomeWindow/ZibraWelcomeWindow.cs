...
namespace com.zibra.liquid.Editor
{
    internal class ZibraWelcomeWindow : EditorWindow
    {
        ...
        // Don't automatically show on pro
        // We have another popup to show instead
...
#if ZIBRA_LIQUID_PRO_VERSION
            int height = 388;
#elif ZIBRA_LIQUID_FREE_VERSION
            int height = 634;
#else
            int height = 609;
#endif
...
#if ZIBRA_LIQUID_PRO_VERSION
            var versionSpecificUSSAssetPath = AssetDatabase.GUIDToAssetPath("69383eef035cb5e4a85d96626f6fcec2");
#elif ZIBRA_LIQUID_FREE_VERSION
        ...
        {
            ZibraWelcomeWindow popup = CreateInstance<ZibraWelcomeWindow>();
            popup.OpenWelcomeWindowInternal();
        }

        private void OpenWelcomeWindowInternal()
        {
            ShowWindow();
        }
    }
