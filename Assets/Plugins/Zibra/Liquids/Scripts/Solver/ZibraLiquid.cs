...
#region Properties
        ...
        /// </remarks>
        public const string PluginVersionStandard = "1.5.3.0";
        ...
        /// <summary>
        ///     Signals whether current Zibra Liquids version is pre-release one.
        /// </summary>
        /// <remarks>
        ...
        /// <summary>
        ///     Zibra Liquids SKU (Pro/Full/Free).
        /// </summary>
#if ZIBRA_LIQUID_PRO_VERSION
        public const string PluginSKU = "Pro";
#elif ZIBRA_LIQUID_PAID_VERSION
        public const string PluginSKU = "Full";
#else
        public const string PluginSKU = "Free";
#endif
        ...
        private ComputeShader RenderCompute;
...
#region Methods
        ...
        public ulong GetSDFsFootprint()
        {
            ...
            foreach (var collider in SDFColliders)
            {
                ...
                if (sdf)
                    result += sdf.GetVRAMFootprint(SolverParameters);
            }
        ...
        /// <summary>
        ///     Return Plugin SKU string ("Free"/"Full"/"Pro")
        /// </summary>
        public static string GetPluginSKUInfo()
        {
#if ZIBRA_LIQUID_PRO_VERSION
            return "Pro";
#elif ZIBRA_LIQUID_PAID_VERSION
...
#region Interop structures
        ...
        private void InitializeParticles()
        {
            ...
            CreateTexture(ref HeightmapTexture, ManipulatorManager.HeightmapSize, FilterMode.Point, 0, RenderTextureFormat.RHalf, false);
#endif
        ...
        private void InitializeSolver()
        {
            ...
            RenderCompute = Resources.Load("RenderCompute") as ComputeShader;
            CopyDepthID = RenderCompute.FindKernel("CS_CopyDepth");
        ...
        private bool SetMaterialParams(Camera cam)
        {
            ...
            if (usingMainMaterial)
            {
                ...
                CurrentMaterial.SetFloat("FresnelStrength", MaterialParameters.FresnelStrength);
                ...
                {
                    CurrentMaterial.EnableKeyword("FLIP_PARTICLES_TEXTURE");
                }
        ...
        private bool CreateTexture(ref RenderTexture texture, Vector2Int resolution,
        ...
        private bool UpdateNativeTextures(Camera cam, float renderPipelineRenderScale)
        {
            ...
            isGlobalTexturesDirty = CreateTexture(ref DepthTexture, cameraResolution, defaultFilter, 32, RenderTextureFormat.RFloat, true)
            ...
            isGlobalTexturesDirty = CreateTexture(ref UpscaleColor, textureResolution, FilterMode.Point, 0,
                                    ...
                                    isGlobalTexturesDirty;
            isGlobalTexturesDirty = CreateTexture(ref ParticlesRT, textureResolution, FilterMode.Point, 0, RenderTextureFormat.ARGBFloat, true)
                   || isGlobalTexturesDirty;
        ...
        internal void RenderLiquidNative(CommandBuffer cmdBuffer, Camera cam, Rect? viewport = null)
        {
            ...
            {
                cmdBuffer.SetComputeVectorParam(RenderCompute, "Resolution", new Vector2(cam.pixelWidth, cam.pixelHeight));
                cmdBuffer.SetComputeTextureParam(RenderCompute, CopyDepthID, "_DepthOUT", DepthTexture);
                cmdBuffer.DispatchCompute(RenderCompute, CopyDepthID, IntDivCeil(cam.pixelWidth, DEPTH_COPY_WORKGROUP), IntDivCeil(cam.pixelHeight, DEPTH_COPY_WORKGROUP), 1);
            }
            ...
            ZibraLiquidBridge.SubmitInstanceEvent(cmdBuffer, CurrentInstanceID, ZibraLiquidBridge.EventID.DrawLiquid);
            if (renderEffectParticles)
            {
                if (SystemInfo.graphicsDeviceType == GraphicsDeviceType.Vulkan)
                {
                ...
                ZibraLiquidBridge.SubmitInstanceEvent(cmdBuffer, CurrentInstanceID, ZibraLiquidBridge.EventID.DrawEffectParticles);
            }
        }
        ...
        private void UpdateCamera(Camera cam)
        {
            ...
            if (ActiveRenderingMode == RenderingMode.MeshRender)
            {
                ...
                CurrentMaterial.EnableKeyword("UNITY_2020_3_WORKAROUND");
#endif
                ...
                {
                    CurrentMaterial.DisableKeyword("FOAM_DISABLED");
                    CurrentMaterial.SetTexture("ParticlesTex", ParticlesRT);
                }
                else
#endif
                {
                    CurrentMaterial.EnableKeyword("FOAM_DISABLED");
                }
            }
