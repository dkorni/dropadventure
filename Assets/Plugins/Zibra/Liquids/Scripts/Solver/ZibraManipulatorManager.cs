...
namespace com.zibra.liquid.Manipulators
{
    internal class ZibraManipulatorManager : MonoBehaviour
    {
        ...
        protected SDFObjectParams GetSDF(SDFObject obj, Manipulator manipulator)
        {
            ...
            sdf.Type = (int)AnalyticSDF.SDFType.Sphere;
            ...
            {
                // use Box as default
                sdf.Type = (int)AnalyticSDF.SDFType.Box; 
            }
        ...
        protected void UpdateHeightmap(CommandBuffer commandBuffer, RenderTexture heightmaps, Terrain terrain, ref int heightmapID, ref SDFObjectParams sdf)
        {
            if (terrain != null)
            {
                ...
                //heightmap is from 0 to 1, but the scales of the objects we have are from -1 to 1, so we need to multiply by 2.0f
        ...
        public void UpdateDynamic(CommandBuffer commandBuffer, ZibraLiquid parent, float deltaTime = 0.0f)
        {
            ...
            foreach (var manipulator in Manipulators)
            {
                ...
                {
                    UpdateHeightmap(commandBuffer, parent.HeightmapTexture, manipulator.GetComponent<Terrain>(), ref heightmapID, ref sdf);
                }
                ...
                if (manipulator is ZibraLiquidVoid)
                {
                    ...
                    // Deletion percentage per frame is set in % / per "ZibraLiquid.DEFAULT_SIMULATION_TIME_SCALE" units of simulation time.
                    // 1 real second = (parent.SimulationTimeScale) units of simulation time.
                    // For example, deletePercentagePerDefaultSimulationTime = 0.5, parent.SimulationTimeScale = 100,
                    // then after 1 real second there should be only (50%) ^ (100 / 40) = 17% particles left.
                    // Therefore, to calculate percentage per frame:
                    //    percentageThatWillLeftPerFrame = pow(deletePercentage, FPS * (1 / simulationTimeScaleDifference)),
                    // and
                    //    deletionPercentagePerFrame = 1 - pow(1 - deletePercentage, 1/FPS * simulationTimeScaleDifference)
                    float simulationTimeScaleDifference = ZibraLiquid.DEFAULT_SIMULATION_TIME_SCALE / parent.SimulationTimeScale;

                    float deltaTimeInRealSeconds = deltaTime / parent.SimulationTimeScale;
                    float deletionPercentagePerFrame = 1.0f - Mathf.Pow(
                        1.0f - liquidVoid.DeletePercentage, deltaTimeInRealSeconds * simulationTimeScaleDifference
                    );
